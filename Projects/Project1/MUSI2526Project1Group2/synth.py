...
def params_CLI():
    ...
    print("1 - Preset 1: Bells")
    print("2 - Preset 2: Buzzy Tremolo")
    print("3 - Custom sound")

    mode = input_valid(range(1, 4))
    ...
    if mode == 1:
        ...
        synth_params['mod_ratio'] = 4
        synth_params['mod_index'] = 6
        synth_params['adsr'] = (0, 20, .2, 80)
        ...
        synth_params['mod_ratio'] = .05
        synth_params['mod_index'] = 100
        ...
        print("1 - Yes")
        print("2 - No")
        filter = input_valid(range(1, 3))
        if filter == 1:
            synth_params['filter'] = True
        elif filter == 2:
            synth_params['filter'] = False

        print("\nAdd reverb?:")
        print("1 - Yes")
        print("2 - No")
        reverb = input_valid(range(1, 3))
        if reverb == 1:
...
def gen_note(freq, dur, amp, synth_params):
    ...
    if synth_params['modulation'] == 'none':
        note = synth_helpers.gen_wave(osc_type, freq, dur, fs, amp)
    if synth_params['modulation'] == 'fm':
        mi = synth_params['mod_index']
        mr = synth_params['mod_ratio']
        note = synth_helpers.fm_synth(osc_type, freq, mi, mr, dur, fs, amp)
    if synth_params['modulation'] == 'am':
        mi = synth_params['mod_index']
        mr = synth_params['mod_ratio']
        note = synth_helpers.am_synth(osc_type, freq, mi, mr, dur, fs, amp)

    return synth_helpers.adsr(note, *synth_params['adsr'], fs=fs)
...
def synth(note_list, synth_params):
    ...
    if synth_params['filter']:
        type = input('Choose Filter Type (lowpass, highpass, bandpass, bandstop): ')
        cutoff_freq = float(input('Set filter cutoff frequency: '))
        order = int(input('Set filter order:'))
        song = synth_helpers.filter(song, type, cutoff_freq, synth_params['fs'], order) 

    if synth_params['reverb']: # remember reverb will be true/false in my example
        ir = input('Insert filepath for impulse convolution or press enter to use default: ')
        if not ir:
            ir = 'IR_medium.wav'
        
        dry_wet = float(input('Set reverb dry/wet mix (0-1): '))
        song = synth_helpers.reverb(song, ir, dry_wet)

    if synth_params['delay']:
        delay_time = float(input('Set Delay Time (ms): '))
        dry_wet = float(input('Set Delay Dry/Wet Mix (0-1): '))
        song = synth_helpers.delay(song, delay_time, dry_wet, synth_params['fs'])
